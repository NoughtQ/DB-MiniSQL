...
dberr_t CatalogManager::CreateTable(const string &table_name, TableSchema *schema, Txn *txn, TableInfo *&table_info) {
  try {
    ...
    if (table_names_.count(table_name) == 1) return DB_TABLE_ALREADY_EXIST;
...
dberr_t CatalogManager::GetTables(vector<TableInfo *> &tables) const {
  ...
  tables.clear();
  // get all table info
  for (const auto &table : tables_) tables.push_back(table.second);
...
dberr_t CatalogManager::CreateIndex(const std::string &table_name, const string &index_name,
  ...
  try {
    ...
    if (index_names_.count(table_name) == 1) {
      ...
      if (index_list.count(index_name) == 1) return DB_INDEX_ALREADY_EXIST;
    }
...
dberr_t CatalogManager::GetIndex(const std::string &table_name, const std::string &index_name,
  ...
  if (index_list.count(index_name) == 0) return DB_INDEX_NOT_FOUND;
...
dberr_t CatalogManager::GetTableIndexes(const std::string &table_name, std::vector<IndexInfo *> &indexes) const {
  ...
  auto index_list = index_names_.at(table_name);
  for (const auto &index : index_list) indexes.push_back(indexes_.at(index.second));
...
dberr_t CatalogManager::DropTable(const string &table_name) {
  ...
  // exception
  if (tables_.count(table_id) == 0) return DB_FAILED;
