...
page_id_t DiskManager::AllocatePage() {
  ...
  uint32_t &num_extents = meta_page->num_extents_;
  if (num_extents == 0 || meta_page->GetExtentUsedPage(num_extents - 1) == BITMAP_SIZE) {
    // 开辟新的 extent
    num_extents++;
  }
  ...
  return (num_extents - 1) * BITMAP_SIZE + page_offset + 1;
}
...
void DiskManager::DeAllocatePage(page_id_t logical_page_id) {
  page_id_t bitmap_id = (logical_page_id - 1) / BITMAP_SIZE * (BITMAP_SIZE + 1) + 1; // 位图页的物理页号
  char bitmap_page_data[PAGE_SIZE] = {0}; // 存储位图的内存
  BitmapPage<PAGE_SIZE> *bitmap_page = reinterpret_cast<BitmapPage<PAGE_SIZE> *>(bitmap_page_data);
  ReadPhysicalPage(bitmap_id, bitmap_page_data); // 读取位图页

  uint32_t page_offset = (logical_page_id - 1) % BITMAP_SIZE; // 分区中的页偏移
...
page_id_t DiskManager::MapPageId(page_id_t logical_page_id) {
  uint32_t num_extents = (logical_page_id - 1) / BITMAP_SIZE; // 计算逻辑页所在的 extent 编号
  uint32_t page_offset = (logical_page_id - 1) % BITMAP_SIZE; // 计算逻辑页在 extent 中的偏移量
